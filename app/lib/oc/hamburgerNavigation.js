...
function hamburgernavigation() {
    ...
    function template() {
        return [
            ...
            '<span>{{\'Order\' | r | xlat}}</span>',
                        '<span ng-if="openOrderCount > 0" class="badge">{{openOrderCount}}</span>',
            '</a>',
            '</li>',
                        //awaiting approval
                        '<li ng-show="waitingOrderCount > 0" ng-class="{\'active\': isActive([\'order\'])}">',
            '<a ng-click="retarget(\'order\')">',
            '<i class="fa fa-thumbs-o-up"></i>',
            '<span>{{\'Awaiting Approval\' | r | xlat}}</span>',
                        '<span ng-if="waitingOrderCount > 0" class="badge">{{waitingOrderCount}}</span>',
            '</a>',
            '</li>',
            //categories
            '<li id="categories">',
